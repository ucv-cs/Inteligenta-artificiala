...
*/
void uniform_cost_search(int start_node, int finish_node) {
	...
	while (!found && node_count > 0) {
		...
		if (current_node == finish_node) {
			...
			// itereaza in linia din matricea de adiacenta, corespunzatoare
			// nodului curent pentru a gasi nodurile succesoare
			for (int i = 0; i < count; i++) {
				// nodurile succesoare neexplorate sunt adaugate la frontiera
				if (matrix[current_node][i] != 0) {
					// costul de la start la nodul curent
					int current_cost =
						matrix[current_node][i] + cost[current_node];

					if (!explored[i] || (current_cost < cost[i])) {
						// daca a fost explorat, il eliminam din frontiera
						if (explored[i]) {
							int j = 0;
							while ((j < node_count) && (frontier[j] != i)) {
								j++;
							}

							if (j < node_count) {
								for (int k = j; k < node_count - 1; k++) {
						...
						// pozitia din frontiera unde trebuie adaugat i
						int j = 0;
						while ((j < node_count) &&
							   (current_cost > cost[frontier[j]])) {
							j++;
						}
						// adaugam i in frontiera
						for (int k = node_count; k > j; k--) {
							frontier[k] = frontier[k - 1];
						}
						frontier[j] = i;
						node_count++;  // actualizeaza numarul de noduri de
									   // explorat
						cost[i] = current_cost;
	...
	cout << "\n\nRezultatul cautarii cu cost uniform: "
...
/**
 @brief Afiseaza lista oraselor pentru ca utilizatorul sa le poata selecta dupa
 id.
 */
void print_problem() {
	cout << "Lista oraselor:\n";
...
int main() {
	...
	while (true) {
		...
		uniform_cost_search(start, finish);
