/**
 * L001. Cautare in latime
 * Aplicati algoritmul de cautare in latime pentru a gasi ruta intre
 * doua orase oarecare.
 */
#include <iomanip>
#include <iostream>
...
/**
 ...
 @param finish_node punct de sosire (index din city_names, definita global)
*/
void breadth_first_search(int start_node, int finish_node) {
	...
	for (int i = path_nodes_count - 1; i >= 0; i--) {
		cout << setw(4) << right << path[i] << ". ";
		cout << setw(15) << left << city_names[path[i]] << endl;
	}
}
...
void print_problem() {
	...
	for (int i = 0; i < count; i++) {
		cout << setw(4) << right << i << ". ";
		cout << setw(15) << left << city_names[i] << "\t";
		if ((i + 1) % 4 == 0) {
			cout << endl;
		}
	}
