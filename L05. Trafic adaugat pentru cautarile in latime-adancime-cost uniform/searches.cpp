/**
 * L05. Trafic adaugat pentru cautarile in latime/adancime/cost uniform
 */
#include <iomanip>
#include <iostream>
using namespace std;
...
void depth_first_search(int, int, float&, int&);
...
*/
void depth_first_search(int start_node, int finish_node, float& time,
						int& cost) {
	int frontier[count] = {start_node};	 // noduri de explorat
	int node_count = 1;					 // numar de noduri de explorat
	bool explored[count] = {false};		 // noduri explorate
