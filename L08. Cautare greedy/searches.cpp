/**
 L08. Pe lângă informațiile anterioare de la tema L007, avem date cu privire la
aproximarea distanței de la fiecare nod la nodul țintă, București, în partea
dreaptă a hărții. Folosiți aceste date pentru a aplica o căutare Greedy pentru a
găsi ruta de la orice oraș către București. Instrucțiuni legate de căutarea
Greedy:
1. Se introduce încă un tablou care să conțină aproximările (notate în general
cu h) de la fiecare nod către București.
2. În tabloul noduri vom ordona elementele în funcție de aproximarea h. Așadar,
un nod i apare înaintea lui j în noduri dacă h[noduri[i]] < h[noduri[j]].
*/
#include <iomanip>
#include <iostream>
#include <string>
...
void greedy_search(int, int, float&, int&);
...
int main(int argc, char* argv[]) {
	...
	while (true) {
		...
		greedy_search(start, finish, time_greedy, cost_greedy);
		...
		while (!exit_flag && depth_constraint <= count) {
			if (depth_first_search(start, finish, time_dfsl, cost_dfsl,
				...
				cout << "GRE  : " << setw(4) << cost_greedy << " km" << setw(10)
					 << get_formatted_time(time_greedy) << '\n';
...
/**
 ...
 @param cost
 */
void greedy_search(int start_node, int finish_node, float& time, int& cost) {
	int frontier[count] = {start_node};	 // noduri de explorat
	int frontier_count = 1;				 // numar de noduri de explorat
	bool explored[count] = {false};		 // noduri explorate
	explored[start_node] = true;		 // marcheaza start ca explorat
	int parent_nodes[count] = {0};		 // noduri-parinte
	bool found = false;
		...
		if (current_node == finish_node) {
			...
			for (int i = 0; i < count; i++) {
				if ((distances[current_node][i] != 0) && !explored[i]) {
					...
					costs[i] = current_cost + distances[current_node][i];
					frontier_count++;	 // actualizeaza numarul de noduri de
										 // explorat
					explored[i] = true;	 // marcheaza nodul succesor ca explorat
					parent_nodes[i] = current_node;
				}
